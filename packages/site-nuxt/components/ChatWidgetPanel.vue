...
const props = defineProps<{
  isOpen: boolean;
  routeFinderTopic?: RouteFinderTopic | null;
...
const isConciergeMode = computed(
  () => conciergeAvailable.value && !isContactRoute.value,
);
const chatEyebrow = computed(() =>
  isContactRoute.value ? "Route finder" : "Website help",
);
...
// The duck marks the start of each assistant run, so consecutive assistant
// bubbles read as one turn without repeating the mascot.
function showsAvatar(index: number): boolean {
  const message = messages.value[index];
  if (!message || message.role !== "assistant") return false;
  const previous = messages.value[index - 1];
  return previous === undefined || previous.role !== "assistant";
}

// Starter chips fill the empty-state under the support welcome and demo the
// routing without typing; they disappear as soon as a conversation exists.
const starterPrompts = [
  "How do repayments work?",
  "Am I eligible for a loan?",
  "I need help with an existing loan",
];

const showStarterChips = computed(() => {
...
function ensureContextWelcome(): void {
  ...
  if (
    ...
    isWelcomeText(firstMessage.text)
  ) {
...
function pushTopicPrimer(topic: RouteFinderTopic): void {
  ...
  if (
    ...
    messages.value.at(-1)?.text === primer
  )
    return;
...
// Smooth scrolling for whole-message arrivals only: per-delta smooth scrolls
// fight each other during streaming, so those stay instant.
...
function applyOfferEligible(telemetry: DemoDisplayTelemetry): boolean {
  ...
  if (telemetry.finalAction !== "answer" || telemetry.safetyFlags.length > 0)
    return false;
...
function connectSupport(): void {
  ...
  void submit(
    "I'd like to talk to a person about my loan application, please.",
    {
      forceEngine: true,
    },
  );
...
async function ensureConciergeSession(): Promise<string> {
  ...
  const session = await $fetch<ConciergeSessionResponse>(
...
async function streamConciergeMessage(
  ...
  if (finalMessage === null || finalMessage.trim() === "") {
    ...
    throw new Error(
      "The assistant reply ended unexpectedly. Please try again.",
    );
  }
  return finalMessage;
}

async function sendConciergeTurn(
  message: string,
  onText: (text: string) => void,
): Promise<string> {
...
onMounted(async () => {
  ...
  if (props.openRequestId > 0) handleOpenRequest();
  try {
    const status = await $fetch<ConciergeStatusResponse>(
