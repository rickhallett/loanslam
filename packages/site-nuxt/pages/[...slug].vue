    ...
    <div class="container section body-grid">
      ...
      <aside class="side">
        <nav
          v-if="showToc"
          class="toc"
          :aria-label="newsCopy.genericPage.tocLabel"
        >
        ...
        <div class="side-cta">
          ...
          <a
            class="btn btn--apply"
            :href="newsCopy.genericPage.sideCta.cta.href"
          >
...
const record = customerContentPages.find(
  (p) => pathFromLink(p.link) === requested,
);
...
const { html, toc } = withToc(
  rewriteLinks(normalizeContentHeadings(record.html)),
);
