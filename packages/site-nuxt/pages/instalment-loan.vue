  ...
  <div>
    ...
    <div class="container section">
      <div class="grid grid--2">
        <div v-for="card in instalmentLoanCopy.cards" :key="card.title" class="card reveal">
          <h3>{{ card.title }}</h3>
          <p>{{ card.body }}</p>
        </div>
      </div>
      ...
      <section class="apply-band reveal">
        ...
        <div class="band-inner">
          ...
          <ol class="steps">
            <li v-for="(step, i) in instalmentLoanCopy.applyBand.steps" :key="i">
              <span class="step-num">{{ i + 1 }}</span>
              <p>{{ step }}</p>
            </li>
          </ol>
          <a class="btn btn--apply" :href="instalmentLoanCopy.applyBand.cta.href">
      ...
      <div class="notice" style="margin-top: 2.5rem" v-html="instalmentLoanCopy.noticeHtml" />
    </div>
  </div>
</template>
...
const items = instalmentQa.map((item) => ({ ...item, a: rewriteLinks(item.a) }));
