...
- **Overshare is a genuine fault line.** Credential-collection blocking is non-negotiable (KEEP, structured guards stay), but the _lexical_ overshare flagging (`validator.test.ts:650`, item 61) is phrase-driven; demote to signal, keep the structured credential block.
...
1. **Demo today: freeze, don't rip.** Two live API sessions already showed acceptable behaviour with current code; do not delete working routing code hours before showing Sam. The discipline is: stop _adding_ static restraints, and rely on live evidence.
